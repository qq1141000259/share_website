 ...
 */
 ...
 router.beforeEach((to, from , next) => {
    // 1. 获取token
    const token = localStorage.getItem('share-website-token')
    ...
    if(!token) {
        ...
        if(to.path === '/login') {
            ...
            const userInfo = localStorage.getItem('share-website-user')
            if(userInfo) {
                // 本地获取到，则直接让它去目标路由
                next()
            }else {
                ...
                getUserInfo(token).then(response => {
                    ...
                    if(resp.flag) {
                        ...
                        // 保存到本地
                        localStorage.setItem('share-website-user', JSON.stringify(resp.data))
                        next()
                    }else {
                        // 未获取到用户信息，重新登录 
                        next({path: '/login'})
